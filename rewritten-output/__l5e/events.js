(()=>{const __name=(target)=>target;((w, version, schemaVersions, maxEventsPerBatch = 50, maxKeepaliveBatchBytes = 50 * 1024, maxReplayBodyBytes = 512 * 1024, maxReplayEventsPerChunk = 100, maxReplayTotalBytes = 5 * 1024 * 1024, maxReplayChunks = 50, maxReplayDurationMs = 10 * 60 * 1e3, replayFlushIntervalMs = 2e3) => {
  ...
  const isInternalPath = /* @__PURE__ */ __name((path) => path === "/__codex_token_check" || path.startsWith("/test-1d/__l5e/"), "isInternalPath");
  ...
  const trackUrl = script?.getAttribute("data-track-url") || "/test-1d/__l5e/trackevents";
  const replayUrl = script?.getAttribute("data-replay-url") || "/test-1d/__l5e/replay";
  const replayScriptUrl = script?.getAttribute("data-replay-script-url") || "/test-1d/__l5e/rrweb-record.js";
  ...
  const stopReplayRecordingOnRuntimeError = /* @__PURE__ */ __name((event) => {
    ...
    const isRecorderError = filename.includes("/test-1d/__l5e/rrweb-record") || stack.includes("/test-1d/__l5e/rrweb-record");
    if (isRecorderError) {
